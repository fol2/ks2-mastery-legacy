...
- full read-only viewer UX in the browser shell beyond the new hub read surfaces
...
The first adult-facing operating surfaces now sit above the repository layer as read models rather than new stores.
...
- hub surfaces consume durable repository data
- they do not own storage
- they do not change subject-engine contracts
- they must not depend on hidden client-only dashboard state
