...
- signed-in zero-writable routing that defaults to Parent Hub, Admin / Operations, or create-first-learner onboarding instead of a fake dashboard path
...
- create-first-learner onboarding is intentionally narrow; it creates one real writable learner but does not add invites, sharing workflow build-out, or provider-account linking
