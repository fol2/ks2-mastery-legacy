...
Signed-in Parent Hub and Admin / Operations now read live Worker hub payloads, can switch between readable learners, and make viewer status explicit. Viewer access still does not join the writable subject shell or learner-write flows.
When the writable bootstrap learner set is empty, the signed-in shell now routes explicitly instead of pretending the dashboard can still act like a writable home: parents with readable memberships go to Parent Hub, admin / ops accounts go to Admin / Operations, and a brand-new parent with no learners gets a create-first-learner onboarding flow.
...
- billing
- messaging
- viewer learners inside the main writable subject shell bootstrap
