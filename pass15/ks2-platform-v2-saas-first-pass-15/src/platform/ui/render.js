...
import { zeroWritableHomeTitle } from '../access/zero-writable-shell.js';
...
function renderHeader(appState, context) {
  ...
  const homeScreen = routeScreen === 'zero-writable' ? 'dashboard' : routeScreen;
    ...
    <header class="card" style="margin-bottom:20px;">
      <div class="card-header">
        ...
        <div class="actions" style="align-items:flex-end; justify-content:flex-end;">
          ...
          ${authChip}
          <button class="btn ${homeScreen === 'dashboard' ? 'ghost' : 'secondary'}" data-action="navigate-home">Dashboard</button>
...
function renderNoWritableLearnerShellCard(context, title = 'No writable learner is selected in the main shell') {
  const detail = context?.shellAccess?.source === 'worker-session'
    ? 'This signed-in account has no writable learner in the main shell. Use the dedicated signed-in home flow instead of treating this like a broken learner-selection state.'
...
function renderCreateFirstLearnerCard(context, state) {
  const form = state?.form && typeof state.form === 'object' ? state.form : {};
  const submitting = Boolean(state?.submitting);
  const learnerName = typeof form.name === 'string' ? form.name : '';
  const yearGroup = typeof form.yearGroup === 'string' && ['Y3', 'Y4', 'Y5', 'Y6'].includes(form.yearGroup) ? form.yearGroup : 'Y5';
  return `
    <section class="card">
      <div class="eyebrow">Signed-in onboarding</div>
      <h2 class="section-title">${escapeHtml(zeroWritableHomeTitle('create-first-learner'))}</h2>
      <p class="subtitle">This parent account is signed in, but it does not have a readable or writable learner yet. Create the first shared learner profile to enter the writable shell. English Spelling is still the only live subject.</p>
      ${state?.notice ? `<div class="feedback warn" style="margin-top:16px;">${escapeHtml(state.notice)}</div>` : ''}
      ${state?.error ? `<div class="feedback bad" style="margin-top:16px;">${escapeHtml(state.error)}</div>` : ''}
      <form data-action="create-first-learner" style="margin-top:18px;">
        <div class="field-row">
          <label class="field">
            <span>Learner name</span>
            <input class="input" name="name" value="${escapeHtml(learnerName)}" data-autofocus="true" autocomplete="off" ${submitting ? 'disabled' : ''} />
          </label>
          <label class="field">
            <span>Year group</span>
            <select class="select" name="yearGroup" ${submitting ? 'disabled' : ''}>
              ${['Y3', 'Y4', 'Y5', 'Y6'].map((value) => `<option value="${value}" ${value === yearGroup ? 'selected' : ''}>${value}</option>`).join('')}
            </select>
          </label>
        </div>
        <div class="small muted" style="margin-top:12px;">This uses the existing shared learner model and repository boundary. Goal, daily minutes and colour start from the current platform defaults and can be edited after creation.</div>
        <div class="actions" style="margin-top:16px;">
          <button class="btn primary" style="background:#3E6FA8;" type="submit" ${submitting ? 'disabled' : ''}>${submitting ? 'Creating learner…' : 'Create first learner'}</button>
        </div>
      </form>
    </section>
  `;
...
function renderZeroWritableScreen(context) {
  const state = context?.zeroWritableState || {};
  const mode = state.mode || (state.status === 'onboarding' ? 'create-first-learner' : 'resolving');
  const platformRole = context?.shellAccess?.platformRole || 'parent';
  const roleLabel = platformRoleLabel(platformRole);

  if (mode === 'create-first-learner' || state.status === 'onboarding') {
    return `
      <section class="two-col">
        ${renderCreateFirstLearnerCard(context, state)}
        <section class="card soft">
          <div class="eyebrow">What this fixes</div>
          <h2 class="section-title">Signed-in zero-writable is now explicit</h2>
          <div class="callout">This is not treated like a broken dashboard any more. The shell stays honest until a real writable learner exists.</div>
          <div class="small muted" style="margin-top:14px;">After learner creation, the account lands in the normal writable shell with that learner selected.</div>
        </section>
      </section>
    `;
  }

  const actionButtons = mode === 'admin-operations'
    ? '<button class="btn secondary" data-action="open-admin-hub">Open Operations</button>'
    : mode === 'parent-readable'
      ? '<button class="btn secondary" data-action="open-parent-hub">Open Parent Hub</button>'
      : '<button class="btn secondary" data-action="navigate-home">Try again</button>';

  const detail = mode === 'admin-operations'
    ? 'This signed-in account has no writable learner in the main shell. Admin / Operations is the honest default surface for this account.'
    : mode === 'parent-readable'
      ? 'This signed-in account has no writable learner in the main shell. Parent Hub is the honest default surface because readable learner memberships exist there.'
      : mode === 'access-error'
        ? 'The requested adult surface is not available for the current platform role.'
        : state?.error
          ? state.error
          : 'The shell is checking whether this signed-in account should land in Parent Hub, Admin / Operations, or first-learner onboarding.';

  return `
    <section class="two-col">
      <section class="card">
        <div class="eyebrow">Signed-in zero-writable home</div>
        <h2 class="section-title">${escapeHtml(zeroWritableHomeTitle(mode))}</h2>
        <p class="subtitle">${escapeHtml(detail)}</p>
        ${state?.notice ? `<div class="feedback warn" style="margin-top:16px;">${escapeHtml(state.notice)}</div>` : ''}
        ${state?.error && mode !== 'error' ? `<div class="feedback bad" style="margin-top:16px;">${escapeHtml(state.error)}</div>` : ''}
        <div class="actions" style="margin-top:18px;">
          ${actionButtons}
        </div>
      </section>
      <section class="card soft">
        <div class="eyebrow">Current account role</div>
        <h2 class="section-title">${escapeHtml(roleLabel)}</h2>
        <div class="callout">The writable subject shell stays separate from readable adult surfaces. Zero-writable accounts do not get fake learner management or misleading subject-launch affordances.</div>
      </section>
    </section>
  `;
}
    ...
    : screen === 'parent-hub'
      ...
      : screen === 'admin-hub'
        ...
        : screen === 'zero-writable'
          ? renderZeroWritableScreen(context)
