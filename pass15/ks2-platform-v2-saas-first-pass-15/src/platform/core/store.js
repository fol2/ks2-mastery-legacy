...
const VALID_ROUTE_SCREENS = new Set(['dashboard', 'subject', 'parent-hub', 'admin-hub', 'zero-writable']);
...
function normaliseRoute(rawRoute, subjects) {
  ...
  if (screen === 'parent-hub' || screen === 'admin-hub' || screen === 'zero-writable') {
...
export function createStore(subjects, { repositories } = {}) {
  ...
  return {
    ...
    openZeroWritable() {
      setState((current) => ({
        ...current,
        route: normaliseRoute({ screen: 'zero-writable' }, registry),
      }));
    },
    setTab(tab) {
      setState((current) => ({
