...
test('signed-in zero-writable parent render shows explicit first-learner onboarding without writable shell leakage', async () => {
  const server = createWorkerRepositoryServer();
  seedAdultAccount(server, {
    id: 'adult-brand-new',
  ...
  const html = renderApp(appState, baseRemoteRenderContext(appState, store, repositories, {
    shellAccess: { platformRole: 'parent', source: 'worker-session' },
    parentHub: null,
    parentHubState: { status: 'error', learnerId: '', error: 'No learner yet.', notice: '' },
    adminHub: null,
    adminHubState: { status: 'idle', learnerId: '', error: '', notice: '' },
  ...
  assert.match(html, /Create your first learner/);
  assert.match(html, /data-action="create-first-learner"/);
  assert.match(html, /Signed-in zero-writable is now explicit/);
  assert.doesNotMatch(html, /data-action="learner-create"/);
  assert.doesNotMatch(html, /data-action="open-subject"/);
