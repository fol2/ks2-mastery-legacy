...
function App() {
  ...
  const subject = route !== 'home' ? subjectsMap[route] : null;
  ...
  const showSidebar = navPattern === 'sidebar' && route !== 'home';
  const showTopbar  = navPattern === 'topbar'  && route !== 'home';
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        {route === 'home' ? (
          <Dashboard onOpenSubject={setRoute} profile={profile} onEditProfile={() => setEditingProfile(true)} />
        ) : (
          <SubjectView
            subject={subject}
            activeTab={tab}
...
function SubjectView({ subject, activeTab, onTabChange, navPattern, onGoHome, density, profile, onEditProfile }) {
  ...
  return (
    <main style={{ padding, maxWidth: max, width: '100%', margin: '0 auto' }}>
      ...
      <SubjectHeader subject={subject} activeTab={activeTab} onTabChange={onTabChange} profile={profile} onEditProfile={onEditProfile} />
      {activeTab === 'practice'  && <PracticeScreen subject={subject} />}
